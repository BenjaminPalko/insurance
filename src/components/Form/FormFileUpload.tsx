...
const FormFileUpload = function ({accept}: FormFileUpload) {
    ...
    return (
        <div className={'flex flex-col gap-5 items-end'}>
            <label
                className="w-48 flex items-center gap-5 px-4 py-6 bg-white dark:bg-gray-800 text-primary-main rounded-lg shadow-lg tracking-wide uppercase border border-primary-main cursor-pointer hover:bg-primary-main hover:text-white">
                ...
                files.length > 0 && (
                    <>
                        <ul className={'flex-1 flex flex-col w-full gap-5'}>
                            {
                                files.map((file, index) => (
                                    <li key={file.name}
                                        ...
                                        <p>{`Type: ${file.type}`}</p>
                                        <button
                                            className={'rounded py-1 px-2 bg-red-800 disabled:bg-gray-700 font-medium'}
                        ...
                        <div className={'w-full flex justify-end gap-3'}>
                            <button
                                className={'rounded py-2 px-3 bg-red-800 disabled:bg-gray-600 font-medium'}
                                onClick={clear}
                                disabled={isUploading}
                            >
                                Clear
                            ...
                            <button
                                className={'rounded py-2 px-3 bg-primary-main disabled:bg-gray-600 font-medium'}
                                onClick={upload}
                                disabled={isUploading}
                            >
                                Upload
